...
class SignalMonitor:
    ...
    async def check_once(self, send_result) -> None:
        for signal in self.storage.open_signals():
            ...
            self.storage.finish_signal(signal.id, status=status, approx_pnl=approx_pnl, real_pnl=real_pnl, result_message_id=result_message_id, mfe_pct=mfe_pct, mae_pct=mae_pct, result_source=classified.result_source)
