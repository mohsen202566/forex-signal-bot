...
class RiskEngine:
    def build_tp_sl(self, *, direction: Direction, entry: float, snapshot_15m: IndicatorSnapshot, levels: LevelsResult, learned_expected_pct: float | None = None, learned_mae_pct: float | None = None) -> RiskResult:
        ...
        learned_sl_distance = entry * learned_mae_pct * 1.35 if learned_mae_pct and learned_mae_pct > 0 else min_sl_distance
        max_sl_distance = max(min_sl_distance * 1.25, entry * MAX_SCALP_SL_PCT)
        rr_floor = max(MIN_RISK_REWARD, 1.10)
        reasons: list[str] = ["TP/SL بر اساس قیمت زنده، ATR، محدوده و حافظه AI ساخته شد."]
        if direction == "LONG":
            ...
            if learned_expected_pct and learned_expected_pct > 0:
                learned_cap = max(min_tp_distance, entry * learned_expected_pct * 1.15)
        ...
        reasons.append("TP/SL اسکالپی قابل اجراست." if ok else "TP/SL فقط برای Watch/یادگیری مناسب است.")
