...
## اصلاح ضد دیر ورود 5M

نسخه فعلی یک فیلتر ساده Anti-Chase دارد تا بعد از حرکت کامل وارد نشود:

- لانگ با RSI بالای 68 رد می‌شود.
- شورت با RSI زیر 32 رد می‌شود.
- اگر قیمت از EMA50 بیشتر از 0.45٪ دور باشد رد می‌شود.
- اگر قیمت از VWAP بیشتر از 0.35٪ دور باشد رد می‌شود.
- اگر حرکت سه کندل اخیر 5M بیشتر از 0.90٪ باشد رد می‌شود.

این حمایت/مقاومت نیست؛ فقط جلوگیری از ورود دیر، خرید روی اوج و شورت روی کف است.
...
این فقط گزارش است و معامله، مانیتورینگ، توبیت، آمار و پنل را تغییر نمی‌دهد.

## آپدیت پنل ترید

در پنل `ترید` وضعیت فعال بودن گزارش `اتو سیگنال` نمایش داده می‌شود:

```text
📡 اتو سیگنال: فعال ✅
```

دستور `اتو سیگنال` همچنان گزارش آخرین اسکن، تعداد ردشده‌ها و دلایل رد شدن ارزها را نشان می‌دهد.
